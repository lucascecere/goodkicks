// TEMPORARY diagnostic — lists Shopify collections + product counts so we can
// find the correct Good Kicks collection handle. Remove after use.
...
export async function GET() {
  ...
  const collections = (json?.data?.collections?.edges ?? []).map(
    (e: { node: { handle: string; title: string; products: { edges: { node: { handle: string } }[] } } }) => ({
      ...
      sample: e.node.products.edges.slice(0, 4).map((p) => p.node.handle),
    }),
  );
